...
public class GivingData : MonoBehaviour
{
	...
	public void AddTagToCharacter(Tag tag)
	{
		...
		if (tag is SpeedDown || tag is SpeedUp || tag is Poison || tag is HitDown || tag is HitUp || tag is CriDown || tag is CriUp || tag is NimDown || tag is NimUp || tag is ConnectDreamTag)
		{
			Tag newTag0 = Instantiate(tag);
			tagList.Add(newTag0);
			BattleSetting.Instance.CheckTagList(this.gameObject);
